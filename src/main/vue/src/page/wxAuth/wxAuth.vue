	...
	export default {
		...
		async created() {
			...
			if(this.$route.query.code) {
                ...
                this.OPENID(res['response'][0]['openId']);

		            // localStorage.setItem('openId', res['response'][0]['openId']);
		            // console.log("openId:"+res['response'][0]['openId']);

// 		            localStorage.setItem('openId', 'oqU2k0qGTEwotyrhvR3F9gRR_hUg');

    				this.$router.replace('/vendings');
